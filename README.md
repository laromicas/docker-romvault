...
|`IONICE_CLASS`| I/O scheduling class for the ROMVault process. Valid values are `0` (none), `1` (realtime), `2` (best-effort), or `3` (idle). Helps manage disk I/O priority, especially useful when running on systems with heavy disk usage. | (no value) |
|`IONICE_LEVEL`| I/O scheduling priority level (0-7) for the ROMVault process, where 0 is the highest priority and 7 is the lowest. Only used when `IONICE_CLASS` is set. Lower values give higher I/O priority. | (no value) |
